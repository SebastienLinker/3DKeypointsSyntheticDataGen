...
def estimate_yaw_angle(kpts, kpts3d):
    ...
    if shoulder_width > exp_width:  # Rounding errors
        logger.debug(f'Shoulder width wider than maximal expected, found {shoulder_width}, max_expected" {exp_width}')
    ...
    left_side_front = rh[2] > lh[2]  # Left side of the body is actually closer to the camera
...
class PositionUpdater():
    ...
    def _add_ankle(self, rotated_kpts3d, idxs: list[int], rotation_angle: float):
        ...
        pt1 = (v_knee_ank[1] * np.cos(rotation_angle)) - (v_knee_ank[2] * np.sin(rotation_angle))
        pt2 = (v_knee_ank[1] * np.sin(rotation_angle)) + (v_knee_ank[2] * np.cos(rotation_angle))
        v_knee_ank[1] = pt1
        v_knee_ank[2] = pt2
...
def simulate_pts(ds, im_folder, display=False):
    ...
    for kpts, kpts3d, im_name, center, scale, K, zind in tqdm.tqdm(zip(ds['part'], ds['S'], ds['imgname'], ds['center'], ds['scale'], ds['K'], ds['zind'])):
